...
print'<form action="">'
f=cgi.FieldStorage()
t1=int(f.getvalue("C1"))
try:
	db=MySQLdb.connect("127.0.0.1","root","","dbjobportal")
	c=db.cursor()
	sql="select * from company_basic where company_id=%d"%(t1)
	try:
		c.execute(sql)
...
#######################################################################################################
print '<center><h2><u>Editing/Deleting Published Vacancies</u></h2></center>'
c_id=int(f.getvalue('C1'))
c1_id=int(f.getvalue('Course'))
d_id=int(f.getvalue('Department'))
post_name=f.getvalue('C2')

# sql2="select vacancy.company_id, department.dept_name, department.dept_id, course.course_name, course.course_id, vacancy.post_name, vacancy.vacancy, vacancy.salary, vacancy.reqd_10_marks, vacancy.reqd_12_marks, vacancy.reqd_graduation_marks, vacancy.experience from vacancy,department,course where vacancy.company_id=%d and vacancy.dept_id=%d and vacancy.course_id=%d and post_name='%s' and vacancy.dept_id=department.dept_id and vacancy.course_id=course.course_id"%(c_id,d_id,c1_id,post_name)
sql5="select vacancy.company_id, department.dept_name, department.dept_id, course.course_name, course.course_id, vacancy.post_name, vacancy.vacancy, vacancy.salary, vacancy.reqd_10_marks, vacancy.reqd_12_marks, vacancy.reqd_graduation_marks, vacancy.experience from vacancy,department,course where vacancy.company_id=%d and vacancy.dept_id=%d and vacancy.course_id=%d and post_name='%s' and vacancy.dept_id=department.dept_id and vacancy.course_id=course.course_id" %(c_id,d_id,c1_id,post_name) 
