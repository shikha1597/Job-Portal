...
name=f.getvalue('t1')
email=f.getvalue('t2')
contact=f.getvalue('t3').strip()
course_id=int(f.getvalue('Course'))
school_t1=f.getvalue('t5')
t1=int(f.getvalue('t6')) 
school_t2=f.getvalue('t7')
t2 = int(f.getvalue('t8'))
college= f.getvalue('t9')
graduation_marks=int(f.getvalue('t10'))
experience=int(f.getvalue('t11'))
password= f.getvalue('t12')
e1=[]
ad1=[]
pn1=[]
pd1=[]
...
cursor=db.cursor()
s1='select email,contact,password from student'
try:
	cursor.execute(s1)
...
if (email in e1):
	...
	sql="insert into student(name,email,contact,course_id,school_10,tenth_marks,school_12,Twelth_marks,college,graduation_marks,experience,password) values('%s','%s','%s',%d,'%s',%d,'%s',%d,'%s',%d,%d,'%s')" % (name,email,contact,course_id,school_t1, t1, school_t2,t2, college,graduation_marks,experience,password)
	try:
		cursor.execute(sql)
		db.commit()
