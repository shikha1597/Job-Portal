...
c=db.cursor()
sql="select * from student where s_id=%d"%(id)	
try:
	c.execute(sql)
...
try:
	...
	em=f.getvalue("t13")
	c=f.getvalue("t14")
	c_id=int(f.getvalue("t15"))
	s_t1=f.getvalue("t16")
	t1=int(f.getvalue("t17"))
	s_t2=f.getvalue("t18")
	t2=int(f.getvalue("t19"))
	s_t3=f.getvalue("t20")
	t3=int(f.getvalue("t21"))
	e=int(f.getvalue("t22"))
	p=f.getvalue("t23")
	
	db=MySQLdb.connect("127.0.0.1","root","","dbjobportal")
	cursor=db.cursor()
	sql3="update student set name= '%s',email='%s',contact='%s',course_id=%d, School_10='%s',tenth_marks=%d,School_12='%s',twelth_marks=%d,college='%s',graduation_marks=%d,experience=%d,password='%s' where s_id=%d" %(n,em,c,c_id,s_t1,t1,s_t2,t2,s_t3,t3,e,p,id)
	try:
		cursor.execute(sql3)
		db.commit()
...
sql2="select student.s_id,student.name,student.email,student.contact,course.course_name, student.school_10,student.tenth_marks,student.school_12,student.twelth_marks,student.college,student.graduation_marks, student.experience,student.password from student,course where s_id=%d and course.course_id=student.course_id"%(id)
try:
	cursor.execute(sql2)
