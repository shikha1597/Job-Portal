...
print '<center><h2><u>Jobs</u></h2></center>'


sql2="select company_basic.company_name, department.dept_name, course.course_name, vacancy.post_name, vacancy.vacancy, vacancy.salary from vacancy, student,course, company_basic,department where vacancy.company_id=company_basic.company_id and vacancy.dept_id=department.dept_id and vacancy.course_id=course.course_id and vacancy.course_id=student.course_id and vacancy.reqd_10_marks<=student.tenth_marks and vacancy.reqd_12_marks<=student.twelth_marks and vacancy.reqd_graduation_marks<=student.graduation_marks and student.s_id=%d"%(t1)
try:
	
	c.execute(sql2)
	resultset=c.fetchall()	
	print "<table>"
	print "<tr align='center'><td><input type='text' value='Company' readonly /></td>"
	print "<td ><input type='text' value='Department' readonly /></td> "
	print "<td><input type='text' value='Degree(required)' readonly /></td> "
	print "<td><input type='text' value='Post Name' readonly /></td>"
	print "<td><input type='text' value='Vacancy' readonly /></td>"
	print "<td><input type='text' value='Salary' readonly /></td></tr></table>"
	for row in resultset:
		id=row[0]
		d=row[1]
		em=row[2]
		...
		s_t1=row[5]	
		print("<table>")
		print "<tr><td><input type='text' name='t11' value='"+id+"' required  readonly /></td>"
		print "<td><input type='text' name='t17' value='"+d+"' required readonly /></td>"	
		print "<td><input type='text' name='t12' value='"+em+"' required readonly /></td>"
		print "<td><input type='text' name='t13' value='"+c+"' required readonly /></td>"
		print "<td><input type='text' name='t14' value=%d"%(c_id)+" required readonly /></td>"
		print "<td><input type='text' name='t16' value=%d"%(s_t1)+" required readonly /></td></tr></table>"		
except Exception as e:
	print(e)
finally:
	db.close()
