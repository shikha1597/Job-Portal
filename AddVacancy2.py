...
c_id=int(f.getvalue('t1'))
d_id=int(f.getvalue('Department'))
c1_id=int(f.getvalue('Course'))
post_name=f.getvalue('t2')
n=int(f.getvalue('t3'))
sal=int(f.getvalue('t4'))
t11=int(f.getvalue('t5'))
t12=int(f.getvalue('t6'))
t13=int(f.getvalue('t7'))
t14=int(f.getvalue('t8'))

sql1="insert into vacancy(company_id,dept_id, course_id, post_name, vacancy, salary, reqd_10_marks, reqd_12_marks, reqd_graduation_marks, experience) values(%d,%d,%d,'%s',%d,%d,%d,%d,%d,%d)" % (c_id,d_id,c1_id,post_name,n,sal,t11,t12,t13,t14)
try:
	c.execute(sql1)
	db.commit()
...
try:
	sql2="select vacancy.v_id, department.dept_name, vacancy.post_name, vacancy.vacancy, vacancy.salary, course.course_name, vacancy.reqd_10_marks, vacancy.reqd_12_marks, vacancy.reqd_graduation_marks, vacancy.experience from vacancy, course, department where vacancy.dept_id=department.dept_id and vacancy.course_id=course.course_id and vacancy.company_id=%d" %(t1)
	try:
		c.execute(sql2)
		resultset=c.fetchall()
		print "<table>"
